***********************************************************************
SimpleMCNP2 - Example class demonstrating how to write new modules for
...
#include <Concrete/SimpleMCNP2.h>
...
/******************************
Methods of class SimpleMCNP2:
******************************/

/***********************************************************************
Constructor for SimpleMCNP2 class. Contains no code except the
definition of its own name.
***********************************************************************/

SimpleMCNP2::SimpleMCNP2(void)
	:BaseModule("SimpleMCNP2")
	{
	}
...
***********************************************************************/
  ...
  Visualization::Abstract::DataSet* SimpleMCNP2::load(const std::vector<std::string>& args, Comm::MulticastPipe* pipe) const
	{
	...
	for(index[0]=0;index[0]<numVertices[0];++index[0]) // I varies most slowly
		for(index[1]=0;index[1]<numVertices[1];++index[1])
			for(index[2]=0;index[2]<numVertices[2];++index[2]) // K varies fastest
				{
				/* Read the vertex position and flux value: */
				double pos[3],flux;
				fscanf(file,"%lf %lf %lf %lf",&pos[0],&pos[1],&pos[2],&flux);
...
extern "C" Visualization::Abstract::Module* createFactory(Plugins::FactoryManager<Visualization::Abstract::Module>& manager)
	...
	Visualization::Concrete::SimpleMCNP2* module=new Visualization::Concrete::SimpleMCNP2();
